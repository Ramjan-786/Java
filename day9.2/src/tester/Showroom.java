package tester;
//Pending case 3 for Purchase vehicle and add the address to the vehicle [Its a HAS-A : tight coupling inner class]
...
//					break;
				case 1:  //Above you we can make two time make object on same data but here write code in better maanner
					if(index < newVelAdd.length) {
						System.out.println("Enter RegNo, color, price, date(day-mon-yr), type");
						//Tester has to invoke method of validationRule : static
						//After write data pass we got object here
				Vehicle v = chkDupVehicle(newVelAdd, sc.nextInt(), sc.next(), sc.nextDouble(), sc.next(), sc.next());
						//After not duplicate found vehicle details added into array
						newVelAdd[index++]=v;
						System.out.println("Vehicle details added...");
					}
